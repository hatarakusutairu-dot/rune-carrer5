...
export const analyzeBalloon = (
  ...
  const summary =
    band === 'low'
      ? 'あなたは不確実な状況で「失敗を避けて確実に成果を出す」傾向が見えました。リスク許容度はやや控えめで、リスク管理が必要な場面で価値を発揮します。実際の海外大手企業の採用検査でも、この傾向は「慎重さ」「品質志向」のサインとして評価されています。'
      : band === 'mid'
        ? 'あなたは状況に応じて踏み込み方を変えられる柔軟性があります。リスク許容度は中程度で、「攻めるべき時は攻め、引くべき時は引ける」バランス型。多くのチームで安定して機能する特性です。'
        : 'あなたは「不確実な状況でも一歩踏み出せる」傾向が強く出ました。リスク許容度は高めで、計算された挑戦ができるタイプ。新しいことを切り拓く役割で力を発揮します。';
  ...
  return {
    gameLabel: '風船リスク課題',
    origin: '世界の大手金融・人材プラットフォーム・日用品メーカーなどの採用で実際に用いられている、行動経済学の「風船リスク課題」をもとにしています。',
