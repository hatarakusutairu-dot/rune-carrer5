...
export const analyzePatternMatch = (
  ...
  const summary = high
    ? 'あなたは Pattern Recognition（パターン認識）と Inductive Reasoning（帰納的推論）が高い水準。Arctic Shores（PwC・Deloitte 等の採用に使用）で測定される指標で、未知の問題から法則を見抜き、戦略を立てる場面で力を発揮します。'
    : mid
      ? 'あなたはパターン推論を着実に使えるタイプ。Arctic Shores の指標としては中程度で、時間をかけて理解を深める学習スタイルが向きます。教育や継続的な専門性が求められる仕事に向きます。'
  ...
  return {
    gameLabel: 'Pattern Match（パターン推論課題）',
    origin: 'Arctic Shores 系のパターン推論課題（PwC・Deloitte 等で採用利用）。認知能力の中でも応用力を測る代表課題',
    trait: 'Pattern Recognition（パターン認識）',
    headline,
    summary,
    strengths,
    realLife,
