...
|---|---|---|
| Phase 0：基盤 | 完了 | 100% |
| Phase 1：ローカル進行版MVP | 完了 | 100% |
| **Phase 2：Kahoot式同期＋採用ゲーム** | **進行中** | **5%** |
| Phase 3：実授業フィードバック | 未着手 | 0% |
| Phase 4：拡張 | 未着手 | 0% |

### Phase 2 Pass進捗
| Pass | 内容 | 状態 |
|---|---|---|
| Pass 1 | 設計書更新 + Worker scaffold + サーバー実装 | 進行中 |
| Pass 2 | クライアント同期層 + ルーム作成・入室 + クラス選択 + リアクション | 未 |
| Pass 3 | 採用ゲーム7種実装 | 未 |
| Pass 4 | 集計・レーダー・分析カード・キャラ競争 | 未 |
| Pass 5 | 演出（入室の森、カウントダウン、待機アニメ） | 未 |
| Pass 6 | AI画像差し込み + 文言調整 | 未 |
| Pass 7 | デプロイ + 実機検証 + 負荷テスト | 未 |
